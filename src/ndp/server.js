...
const debug = createDebugLogger('devp2p:ndp:server')
const VERSION = 0x01
const createSocketUDP4 = dgram.createSocket.bind(null, 'udp4')

class Server extends EventEmitter {
  constructor (ndp, privateKey, options) {
    super()
    this._ndp = ndp
  ...
  _send (peer, typename, data) {
    ...
    // Parity hack
    // There is a bug in Parity not echoing the hash from
  ...
  _handler (msg, rinfo) {
    ...
    const peer = this._ndp.getPeer(peerId)
    ...
    switch (info.typename) {
      ...
      case 'findneighbours':
        ...
        this._send(rinfo, 'neighbours', {
          peers: this._ndp.getClosestPeers(info.data.id)
        })
        break
