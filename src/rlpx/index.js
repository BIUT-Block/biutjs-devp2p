...
class RLPx extends EventEmitter {
  constructor (privateKey, options) {
    ...
    this._clientId = Buffer.from(options.clientId || `secjs-devp2p/v${pVersion}/${os.platform()}-${os.arch()}/nodejs`)
    ...
    if (this._ndp !== null) {
      ...
      this._ndp.on('peer:removed', (peer) => {
        // remove from queue
